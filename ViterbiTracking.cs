...
namespace MeetingAgent
{
    ...
    class ViterbiTracking
    {
        ...
        {
            public int cost;
            public RecognisedObject sourceObject;
        }
        ...
        public string process(List<List<RecognisedObject>> objectsByWorldState)
        {
        ...
        public string getPaths(List<List<RecognisedObject>> objectsByWorldState)
        {
            string output = "";

            // sort the elments by ascending cost
            objectsByWorldState[objectsByWorldState.Count - 1].Sort(delegate (RecognisedObject c1, RecognisedObject c2) { return c1.cost.CompareTo(c2.cost); });
            
            foreach (RecognisedObject objectAtWorldState in objectsByWorldState[objectsByWorldState.Count - 1]) 
            {
                int iWorldState = objectsByWorldState.Count;
                // temporary object to iterate through the linked list
                RecognisedObject thisObject = objectAtWorldState;                
                do
                {
                    iWorldState--;
        ...
        public string displayMatrix(List<List<RecognisedObject>> objectsByWorldState)
        {
            string output = "";
            int iWorldState = 0;
            foreach (List<RecognisedObject> objectsAtWorldState in objectsByWorldState)
            {
                output += "--- WorldState " + iWorldState + " ---" + Environment.NewLine;
                int iDestinyObject = 0;
                foreach (RecognisedObject destinyObject in objectsAtWorldState)
                {
                    ...
                    if (iWorldState > 0)
                    {
                        ...
                        List<RecognisedObject> objectsAtPreviousWorldState = objectsByWorldState[iWorldState - 1];
                        int iSourceObject = 0;
                        foreach (RecognisedObject sourceObject in objectsAtPreviousWorldState)
                        {
        ...
        private void calculateMinimumCumulativeCost(List<List<RecognisedObject>> objectsByWorldState)
        {            
            int iWorldState = 0;
            foreach (List<RecognisedObject> objectsAtWorldState in objectsByWorldState)
            {
        ...
        /// </summary>
        /// <param name="objectsByWorldState">"Matrix" of objects by world state</param>
        private void calculatePairwisteCost(List<List<RecognisedObject>> objectsByWorldState)
        {
            int dX, dY, distance;
        ...
        private int objectWithShortestPathAtWorldState(List<RecognisedObject> objectsAtWorldState) {
        ...
        /// <returns></returns>
        private ObjectAndCost minCost(List<RecognisedObject> objectsAtPreviousWorldState, int iWorldState, int iObject)
        {
            ...
            int iSourceObject = 0;
            foreach (RecognisedObject thisObject in objectsAtPreviousWorldState)
            {
        ...
        private int getMaxObjectsInWorldStates(List<List<RecognisedObject>> objectsByWorldState)
        {
            int max = 0;
            foreach (List<RecognisedObject> objectsAtWorldState in objectsByWorldState)
            {
